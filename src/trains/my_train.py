...
def train_and_evaluate(X_my, Y_my, reports_dir, models_dir):
    ...
    X_train_my, X_test_my, Y_train_my, Y_test_my = train_test_split(
        X_my, Y_my, test_size=0.20, random_state=42, stratify=Y_my
    )
    ...
    disp_base_my = ConfusionMatrixDisplay(
        confusion_matrix=cm_base_my, display_labels=["Churn (0)", "Churn(1)"]
    )
    ...
    disp_rf_my = ConfusionMatrixDisplay(
        confusion_matrix=cm_rf_my, display_labels=["Churn(0)", "Churn(1)"]
    )
    ...
    ax.plot(
        ...
        linestyle="--",
        linewidth=1.5,
        label="Rastgele Tahmin (AUC = 0.5000)",
    )
    ...
    ax.set_xlabel(
        "Yalancı Alarm Oranı (False Positive Rate)",
        color=text_color,
        fontsize=11,
        labelpad=10,
    )
    ax.set_ylabel(
        "Doğru Yakalama Oranı (True Positive Rate)",
        color=text_color,
        fontsize=11,
        labelpad=10,
    )
    ...
    legend = ax.legend(
        loc="lower right", facecolor="#252525", edgecolor="#333333", fontsize=10
    )
    ...
    ax.tick_params(colors=text_color, which="both", labelsize=11)
    ax.set_xlabel(
        "Önem Derecesi (0 - 1 Arası)", color=text_color, fontsize=11, labelpad=10
    )
    ax.set_title(
        "Modelin Karar Verirken En Çok Önem Verdiği Özellikler",
        color=text_color,
        fontsize=13,
        fontweight="bold",
        pad=20,
